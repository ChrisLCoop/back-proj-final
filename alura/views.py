...
from .models import (
    Categoria,Area,Curso,CursoImagen,Comentario
)
...
from alura.forms import Comentarioformulario
...
def comentarios(request):
    ...
    context = {
        'context':lista_comentario,
    }
    return render(request,'curso.html',context)
    
def detalle_curso(request, curso_id):
    # Obtener los comentarios asociados al curso
    comentarios = Comentario.objects.filter(id_curso=curso_id)
    # Pasar los comentarios a la plantilla
    return render(request, 'detalle_curso.html', {'comentarios': comentarios})

class cursodetallevista(generic.DetailView):
    model = Curso
...
class cursocomentariovista(LoginRequiredMixin, SingleObjectMixin, FormView):
    template_name = 'curso.html'
    form_class = Comentarioformulario
    model = Curso
...
class Cursovista(View):
    ...
    def get(self, request, *args, **kwargs):
        view = cursodetallevista.as_view()
        return view(request, *args, **kwargs)
        
    def post(self, request, *args, **kwargs):
        view = cursocomentariovista.as_view()
        return view(request, *args, **kwargs)
